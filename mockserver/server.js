...
app.get('/v10/details/:aggregate/:zoneId', (req, res, next) => {
  const { aggregate, zoneId } = req.params;

  // if file exists return it, otherwise redirect to DEFAULT file
  if (fs.existsSync(`./public/v10/details/${aggregate}/${zoneId}.json`)) {
    // file structure of project will return the correct file
    next();
  } else {
    res.redirect(`/v10/details/${aggregate}/${DEFAULT_ZONE_KEY}`);
  }
});

app.get('/v10/gfs/wind', (req, res, next) => {
  const { refTime, targetTime } = req.query;

  fs.readFile(`./public/v10/gfs/wind.json`, (err, data) => {
...
app.get('/v10/gfs/solar', (req, res, next) => {
  const { refTime, targetTime } = req.query;

  fs.readFile(`./public/v10/gfs/solar.json`, (err, data) => {
    const jsonData = JSON.parse(data);
    jsonData.data[0].header.refTime = targetTime;
